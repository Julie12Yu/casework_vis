...
def download_pdfs_from_drive(service, folder_id=None, max_pages=20, last_pages=3):
    """
    Download all PDFs from specified folder
    Extract text from first max_pages and last last_pages
    If document is short enough, just extract all pages
    """
...
def generate_summaries(documents, client, delay=1.0):
    """
    Generate summaries for all documents using OpenAI API
    GPT-4o can handle 128K tokens (~400K characters)
    We use the ENTIRE document text for complete analysis
    """
    summaries = []
    
    for i, doc in enumerate(documents):
        # Use the ENTIRE document text - no truncation
        # GPT-4o has 128K token context, which is ~400K characters
        # Most court opinions will fit entirely
        text = doc['text']
...
def save_to_json(documents, summaries, output_file):
    ...
    print(f"\n✓ Saved {len(data)} documents with summaries to {output_file}")
...
def main():
    print("=" * 60)
    print("STEP 1: Download PDFs and Generate Summaries")
    print("=" * 60)
